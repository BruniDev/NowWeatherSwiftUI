...
public class WeatherManager : ObservableObject {
    ...
    func requestWeatherForCurrentLocation() async {
        ...
        do {
            ...
            weatherDefault?.set(weather?.dailyForecast[1].highTemperature.value.roundCelcius(),forKey: "tomorrowHighTemp")
            weatherDefault?.set(weather?.dailyForecast[1].lowTemperature.value.roundCelcius(),forKey: "tomorrowLowTemp")
            weatherDefault?.set(weather?.dailyForecast[1].condition.description,forKey: "tomorrowCondition")
